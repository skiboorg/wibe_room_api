...
class InfoProductListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'cover', 'short_description', 'price', 'is_favorite'
        ]
    ...
    def get_is_favorite(self, obj):
        ...
        return Favorite.objects.filter(user=request.user, product_id=obj.pk).exists()
...
class InfoProductDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'product_info', 'files', 'main_link', 'is_favorite'
        ]
...
class InfoProductCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = InfoProduct
        fields = [
            'id', 'is_main', 'title', 'slug', 'community',
            'cover', 'short_description', 'price',
            'product_info', 'product_info_structure',
            'files', 'main_link',
            '_new_files', '_new_files_names',
            '_delete_file_ids', '_update_file_ids', '_update_file_names',
        ]
        ...
        if community and title:
            if self.instance is None:
                if InfoProduct.objects.filter(title=title, community=community).exists():
                    raise serializers.ValidationError({'title': 'Продукт с таким названием уже существует в этой группе'})
            else:
                if InfoProduct.objects.filter(title=title, community=community).exclude(id=self.instance.id).exists():
                    raise serializers.ValidationError({'title': 'Продукт с таким названием уже существует в этой группе'})
        return attrs
